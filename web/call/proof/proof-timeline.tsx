  ...
  const receiveRag = useCallback(
    (message: { payload: Uint8Array }) => {
      try {
        ...
        const incoming: ReferencedChapter[] = rawSources
          .filter(
            (source): source is ReferencedChapter =>
              typeof source === "object" &&
              source !== null &&
              typeof (source as ReferencedChapter).citation === "string",
          )
          .map((source) => ({
        ...
        const event = ragEventFromMessage(body);
