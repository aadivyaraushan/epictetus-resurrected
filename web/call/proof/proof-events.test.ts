...
describe("proof-call events", () => {
  ...
  it("reads grounded and rejected RAG decisions from the existing source topic", () => {
    ...
    expect(rejected?.detail).toContain("0.310 cosine");
    expect(rejected?.detail).toContain("Luna rejected");
    expect(rejected?.detail).toContain("inside Luna range");
  });
  ...
  it("makes skipped and failed retrieval visible instead of silently omitting them", () => {
    ...
    expect(failed).toMatchObject({
      category: "rag",
      title: "RAG could not ground this turn",
    });
